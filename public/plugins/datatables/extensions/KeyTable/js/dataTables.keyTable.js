...
KeyTable = function ( oInit )
{
		...
		switch( iKey )
		{
			...
			case 37: /* left arrow */
				if ( _iOldX > 0 ) {
					...
					if ( iKey == -1 && _bForm )
					{
						/* If we are in a from, return focus to the 'input' element such that tabbing will
	...
	function _fnInit( table, datatable, oInit, that )
	{
		...
		/* If the table is inside a from, then we need a hidden input box which can be used by the

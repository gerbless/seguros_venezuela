...
var ZeroClipboard_TableTools = {
	...
	dispatch: function(id, eventName, args) {
		// receive event form flash movie, send to client
...
TableTools = function( oDT, oOpts )
{
	...
	// In 1.10 we can use the API to get the settings object form a number of
	// sources
	...
	this.s = {
		/**
		 * Store 'this' so the instance can be retrieved form the settings object
		 * @property that
		 * @type	 object
		 * @default  this
		 */
	...
	this.dom = {
		...
		"print": {
			/**
			 * Nodes which have been removed form the display by setting them to display none
			 *  @property hidden
			 *  @type	 array
			 *  @default  []
			 */
	...
	/**
	 * Retreieve the settings object form an instance
	 *  @method fnSettings
...
TableTools.prototype = {
	...
	/**
	 * Get the title of the document - useful for file names. The title is retrieved form either
	...
	{
		/* In future this could be used to get data form a plain HTML source as well as DataTables */
		if ( this.s.dt )
		{
	...
	"_fnCustomiseSettings": function ( oOpts )
	{
		...
		/* We can use the table node form comparisons to group controls */
			...
			$(dt.nTBody).on( 'click.DTTT_Select', this.s.custom.sRowSelector, function(e) {
				...
				if ( select.type == 'os' ) {
					if ( e.ctrlKey || e.metaKey ) {
						// Add or remove form the selection
						...
						// Add a range of rows, form the last selected row to
						// this one
						...
						if ( that.fnGetSelected().length === 0 || idx1 === -1 ) {
							// select form top to here - slightly odd, but both
	 ...
	 *     a jQuery object), a single aoData point form DataTables, an array of aoData
	...
	"_fnPrintScrollStart": function ()
	{
		 ...
		 * in print view. Note that this section of code is more or less verbatim form DT 1.7.0
		 */
 ...
 *     4. string - Returned string form Flash (flash button only - and only on 'complete')
 */

<div class="box box-primary">
    <div class="box-header">
        <h4 class="box-title">
            Listados de Ocupaciones
        </h4>
    </div>
    <div class="box-body">
        <div class="col-lg-12">
            <button class="btn btn-default" onclick="cargarlistado('create-ocupacion');">
        ...
        @if(count($data) >  0)
            ...
            <table id="tabla_banco" class="display table table-hover" cellspacing="0" width="100%">
                ...
                @foreach($data as $item)
                    <tr>
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->nb_ocupacion}}</td>
                        <td>{{ $item->status->nb_status }}</td>
                        <td>
                            <button class="btn  btn-info btn-xs" onclick="cargarlistado('editar-ocupacion/{{$item->id}}');" ><i class="fa fa-plus"></i> EDIT</button>
                        </td>
                    </tr>
                @endforeach

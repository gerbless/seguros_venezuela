<div class="box box-primary">
    ...
    <div class="box-body">
        <div class="col-lg-12">
            <button class="btn btn-default" onclick="cargarlistado('create-productos');">
        ...
        @if(count($data) >  0)
            ...
            <table id="tabla_productos" class="display table table-hover" cellspacing="0" width="100%">
                ...
                @foreach($data as $item)
                    <tr>
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->nb_producto }}</td>
                        <td>{{ $item->ramos->nb_ramo}}</td>
                        <td>{{ $item->status->nb_status }}</td>
                        <td>
                            <button class="btn  btn-info btn-xs" onclick="cargarlistado('editar-productos/{{$item->id}}');" ><i class="fa fa-plus"></i> EDIT</button>
                        </td>
                    </tr>
                @endforeach
